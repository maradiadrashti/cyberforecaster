        ...
        <nav className="flex-1 py-3 px-2 flex flex-col gap-3 overflow-y-auto">
          {NAV_GROUPS.map((group) => (
            <div key={group.title} className="space-y-0.5">
              {!sidebarCollapsed && (
                <div className="px-3 py-1 text-[9px] font-mono-tech font-extrabold tracking-widest text-slate-500 uppercase">
                  {group.title}
                </div>
              )}
        ...
        <header className="border-b border-cyber-border bg-[#080b12]/90 backdrop-blur-md px-6 py-3 flex justify-between items-center sticky top-0 z-40">
          ...
          <div className="flex items-center gap-3">
            {activePage === "live" && (
              <>
                ...
                <div className="flex items-center gap-1.5 bg-slate-900/80 border border-slate-800 px-2.5 py-1 rounded-full text-[9px] font-mono-tech">
                  ...
                  <span className="text-slate-400">{highThreatAlerts} Flows</span>
                </div>
                <div className="flex items-center gap-1.5 bg-slate-900/80 border border-slate-800 px-2.5 py-1 rounded-full text-[9px] font-mono-tech">
              ...
              <>
                <div className="flex items-center gap-1.5 bg-slate-900/80 border border-slate-800 px-2.5 py-1 rounded-full text-[9px] font-mono-tech">
                  ...
                  <span className="text-slate-400">
                    Source: {offlineAnalysisResult ? (offlineAnalysisResult.file_type === "csv" ? "OFFLINE CSV" : "OFFLINE PCAP") : "LIVE TELEMETRY"}
                  </span>
                </div>
                <div className="flex items-center gap-1.5 bg-purple-950/40 border border-purple-800/50 px-2.5 py-1 rounded-full text-[9px] font-mono-tech text-purple-300">
                  ...
                  <span>PYTORCH GRU FORECAST</span>
                </div>
              </>
            )}
            ...
            {activePage === "upload" && (
              <>
                <div className="flex items-center gap-1.5 bg-slate-900/80 border border-slate-800 px-2.5 py-1 rounded-full text-[9px] font-mono-tech">
                  <UploadCloud className="h-3 w-3 text-cyan-400" />
                  <span className="text-slate-400">Supported: .pcap, .pcapng, .csv</span>
                </div>
                <div className="flex items-center gap-1.5 bg-cyan-950/40 border border-cyan-800/50 px-2.5 py-1 rounded-full text-[9px] font-mono-tech text-cyan-300">
                  <Server className="h-3 w-3 text-cyan-400" />
        ...
        <main className="flex-1 overflow-y-auto p-6 animate-fade-in">
          ...
          <div style={{ display: activePage === "forecast" ? "block" : "none" }}>
            <AttackForecast
              ...
              onClearOffline={handleClearOfflineAnalysis}
            />
          </div>
